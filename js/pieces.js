    ...
    if(file.archived){
        archivedInfo = `
            <div class="summons-meta">
                <div><span>تاريخ الإضافة</span><b>${file.createdAt?new Date(file.createdAt).toLocaleDateString("ar-DZ"):"-"}</b></div>
                ${file.deliveryDate?`<div><span>تاريخ التسليم</span><b>${file.deliveryDate}</b></div>`:""}
                ${file.finishedDate?`<div><span>تاريخ الإنهاء</span><b>${file.finishedDate}</b></div>`:""}
